...
class Scheduler:    
    def __init__(self, teams) -> None:
        ...
        self.totalmatches = ((self.n // 2) * (self.n - 1))*2 if self.teams % 2 == 0 else (((self.n // 2) - 1) * (self.n - 1))*2
        self.cols = self.n // 2
        self.rows = self.n - 1
        self.table1 = []
        self.table2 = []
    ...
    def merge_tables(self):
        ...
        for row1, row2 in zip(self.table1, self.table2):
            merged_row = [[cell1, cell2,0] for cell1, cell2 in zip(row1, row2)]
    ...
    def update_first_column(self):
        ...
        for i in range(len(self.merged_table)):
            if i % 2 == 0:
                self.merged_table[i][0] = [player_or_bye, self.merged_table[i][0][1], 0]
            else:
                self.merged_table[i][0] = [self.merged_table[i][0][0], player_or_bye, 0]
    ...
    def new_face_off(self):
        ...
        for row in self.merged_table:
            new_row = []
            for match in row:
                new_row.append([match[1],match[0],0])
